 ...
 * 
 * All modules are available on ALL tiers. Access is controlled by
 * quantity limits (MODULE_LIMITS), not tier-gating. Every user can
 * activate every module — they just hit creation limits on Free.
 */

import { MODULE_DEFINITIONS, MODULE_LIMITS, TIER_CONFIG } from '../constants.js';
...
/**
 * Get the free-tier limit for a module (for display on the activation card)
 */
function getFreeTierLimit(moduleId) {
  const limits = MODULE_LIMITS.free;
  const limit = limits[moduleId];
  if (limit === undefined) return null;
  if (limit === Infinity) return 'Unlimited';
  
  // Special case: gallery shows "1 album (5 photos)"
  if (moduleId === 'gallery') return `${limit} album (${limits.galleryPhotos || 5} photos)`;
  // Special case: announcements shows "1 active"
  if (moduleId === 'announcements') return `${limit} active`;
  return `${limit}`;
}

/**
 * Render a full-page activation card (shown when navigating to an inactive module)
 * No paywall — all modules are activatable on every tier.
 */
export function renderModuleActivationPage(moduleId) {
  const mod = MODULE_DEFINITIONS[moduleId];
  if (!mod) return '<p>Unknown module</p>';

  const freeLimit = getFreeTierLimit(moduleId);
  const limitHint = freeLimit 
    ? `Free plan includes up to ${freeLimit} ${mod.label.toLowerCase()}. Upgrade anytime for more.`
    : '';
    ...
    <div class="module-activation">
      <div class="module-activation__card">
        ...
        <p class="module-activation__hint">
          ${limitHint || `This will add ${mod.label.toLowerCase()} management to your dashboard. Your PDM team may need to add the corresponding section to your website.`}
        </p>
      </div>
    </div>
  `;
 ...
 * No paywall — all modules show the "Add" button regardless of tier.
 */
  ...
  return `
    <div class="grow-card" data-module="${moduleId}">
      ...
      <div class="grow-card__info">
        ...
        ${freeLimit ? `<span class="grow-card__limit">Free: ${freeLimit}</span>` : ''}
      </div>
