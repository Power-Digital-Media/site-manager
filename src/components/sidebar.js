...
const NAV_ICONS = {
  ...
  menu:  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 6h18M3 12h18M3 18h18"/><circle cx="7" cy="6" r="1.5" fill="currentColor"/><circle cx="7" cy="12" r="1.5" fill="currentColor"/><circle cx="7" cy="18" r="1.5" fill="currentColor"/></svg>',
};

// Known restaurant site IDs (fallback when industry field wasn't set at onboarding)
...
function getNavItems() {
  ...
  // Restaurant sites get a dedicated Menu Manager link
  const isRestaurant = site?.industry === 'restaurant' || RESTAURANT_SITE_IDS.includes(site?.id);
  if (isRestaurant) {
    items.push({ id: 'menu', label: '🌶️ Menu Manager' });
  }
